...
class AMR(object):
    ...
    def __init__(self, node_list=None, node_value_list=None, relation_list=None, attribute_list=None, reent=None, allrelations=None):
        ...
        for dct in attribute_list:
            ...
            for item in dct:
                if len(dct[item]) > 1 and dct[item][0].endswith("_"):
                    dct[item] = ('"' + dct[item][0][0:-1] + '"', dct[item][1])
                dct2[item] = dct[item]
        ...
        for r in allrelations:
            if len(r[2]) > 1 and r[2][0].endswith("_"):
                r2 = ('"' + r[2][0][0:-1] + '"', r[2][1])
                allrelations2.append((r[0], r[1], r2))
            else:
    ...
    def parse_AMR_line(line, normalize_inv = True):
        ...
        for i, c in enumerate(line.strip()):
            if c == " ":
                ...
                if state == 2:
                    if len(cur_charseq) == 0: start_charseq = i
            ...
            if c == "\"":
                ...
                if state == 3:
                    ...
                    node_dict[cur_node_name] = (node_value, start_charseq)
                # Last significant symbol is ":". Now we encounter ":"
                # Example:
                # :op1 w :quant 30
                # or :day 14 :month 3
                ...
                # or node value (variable) at this moment
                    ...
                    if relation_name.endswith("-of") and normalize_inv:
                        # if stack[-1] not in node_name_list:
                        allrelations.append((relation_value,relation_name[:-3],(stack[-1], start_charseq + len(relation_name) + 1)))
                        # else:
                        ...
                        # if relation_value not in node_name_list:
                        allrelations.append((stack[-1],relation_name,(relation_value, start_charseq + len(relation_name) + 1)))
                        # else:
