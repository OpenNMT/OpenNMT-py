...
class StackedSREncoder(nn.Module):
    def __init__(self, layers, input_size, hidden_size, dropout):
        ...
        for _ in range(layers):
            self.sgus.append(
                onmt.modules.SRUnits.BiSRU(input_size, hidden_size, dropout))
...
class SRDecoder(nn.Module):
    ...
    def forward(self, input, context, hidden, memory_lengths):
        ...
        outputs, hidden, attn = self.stacked(rnn_outputs,
                                             hidden, context,
                                             memory_lengths)
...
class StackedSRDecoder(nn.Module):
    def __init__(self, num_layers, input_size, rnn_size, dropout):
        ...
        self.dropout = nn.Dropout(dropout)
        self.state_class = onmt.modules.SRUnits.SRDecoderState
    ...
    def forward(self, dec_state, hidden, enc_out, memory_lengths):
        ...
        for i, layer in enumerate(self.layers):
            input, new_hidden, attn_state = layer(input, hidden._all[i],
                                                  enc_out, memory_lengths)
            hiddens += [new_hidden]

        return self.dropout(input), \
            self.state_class(input.size()[-1],
                             torch.stack(hiddens)), \
            attn_state
