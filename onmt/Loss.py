...
class Statistics:
    ...
    def score(loss, scores, targ):
        pred = scores.max(1)[1]
        non_padding = targ_t.ne(onmt.Constants.PAD)
...
#     def loss(self, out, score, targ):
#         scores = self.generator(out)
#         loss = self.crit(scores, targ.view(-1))
#         return loss, scores
...
#         for s in splitter.split(original):
