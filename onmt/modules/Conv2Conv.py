...
class GatedConv(nn.Module):
    def __init__(self, input_size, width=3, dropout=0.2,nopad = False):
        ...
        self.conv = WN_Conv2d(input_size, 2 * input_size, kernel_size=(width,1), stride=(1,1), padding=(width//2 * (1 - nopad), 0))
...
class ConvDecoder(nn.Module):
    ...
    def __init__(self, hidden_size, layers, opt):
        ...
        for i in range(self.n_layers):
            self.conv_layers.append(GatedConv(self.hidden_size, opt.width, opt.dropout, True))

        self.attn_layers = nn.ModuleList()
        for i in range(self.n_layers):
            self.attn_layers.append(onmt.modules.ConvAttention(self.hidden_size))


    def forward(self, target_emb, encoder_out_t, encoder_out_c):
        emb_reshape = target_emb.contiguous().view(target_emb.size(0) * target_emb.size(1),-1)
        ...
        for conv, attention in zip(self.conv_layers,self.attn_layers):
            ...
            c, attn = attention(base_target_emb, out, encoder_out_t, encoder_out_c)
