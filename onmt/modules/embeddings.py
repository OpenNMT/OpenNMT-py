...
def read_embeddings(path, skip_lines=0, filter_set=None, randomize=False):
    """
    ...
    with open(path, "rb") as f:
        for i, line in enumerate(f):
            ...
            if filter_set is not None and l_split[0] not in filter_set:
                missing.append(l_split)
                continue
            embs[l_split[0]] = [float(em) for em in l_split[1:]]
            del filter_set[l_split[0]]

    logger.info(f"NUM LINES: {num_lines}")
    logger.info(f"FILTER SET: {len(filter_set.keys())}")

    if randomize:
        for key in filter_set.keys():
            if len(missing) > 0:
                embs[key] = [
                    float(em) for em in missing.pop(random.randrange(len(missing)))[1:]
                ]
