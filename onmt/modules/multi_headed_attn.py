    ...
    def forward(self, key: Tensor, value: Tensor,
        ...
        if not self.layer_cache["empty"]:
            if self.attn_type == "self":
                ...
                if self.layer_cache["self_keys"] is not None:
                    key = torch.cat(
                        (self.layer_cache["self_keys"], key),
                        dim=2)

                if self.layer_cache["self_values"] is not None:
                    value = torch.cat(
                        (self.layer_cache["self_values"], value),
                        dim=2)
                self.layer_cache["self_keys"] = key
                self.layer_cache["self_values"] = value
            elif self.attn_type == "context":
                query = self.linear_query(query)
                if self.layer_cache["memory_keys"] is None:
                    key, value = self.linear_keys(key),\
                                 self.linear_values(value)
                    key = shape(key, self.dim_per_head)
                    value = shape(value, self.dim_per_head)
                else:
                    key, value = self.layer_cache["memory_keys"],\
                               self.layer_cache["memory_values"]
                self.layer_cache["memory_keys"] = key
                self.layer_cache["memory_values"] = value
        else:
        ...
        if self.relative_positions_embeddings is not None:
            ...
            relative_positions_matrix = gen_relative_positions(
                ...
                cache=True if self.layer_cache is not None else False,
                device=key.device)
