...
class MultiHeadedPseudoSelfAttention(nn.Module):
    def __init__(
        self, head_count, model_dim, dropout=0.1, max_relative_positions=0
    ):
        assert model_dim % head_count == 0
        ...
        super(MultiHeadedPseudoSelfAttention, self).__init__()
        self.head_count = head_count
    ...
    def forward(self, src, tgt, mask=None, layer_cache=None, attn_type=None):
        batch_size = tgt.size(0)
        dim_per_head = self.dim_per_head
        head_count = self.head_count
        key_len = tgt.size(1)
        query_len = tgt.size(1)
