...
class BertModelSaver(ModelSaverBase):
    """Simple model saver to filesystem"""
    ...
    def _save(self, step, model):
        ...
        for side in ["tokens"]:
            ...
            # if hasattr(vocab[side], "fields"):
            #     unk_token = vocab[side].fields[0][1].vocab.itos[0]
            #     for key, value in vocab[side].fields[0][1]
            #                       .vocab.stoi.items():
            #         if value == 0 and key != unk_token:
            #             keys_to_pop.append(key)
            #     for key in keys_to_pop:
            #         vocab[side].fields[0][1].vocab.stoi.pop(key, None)
            unk_token = vocab[side].unk_token
            unk_id = vocab[side].vocab.stoi[unk_token]
            for key, value in vocab[side].vocab.stoi.items():
                if value == unk_id and key != unk_token:
                    keys_to_pop.append(key)
                for key in keys_to_pop:
                    vocab[side].vocab.stoi.pop(key, None)
