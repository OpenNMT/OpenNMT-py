...
$my_python train.py -config data/lm_data.yaml -save_model /tmp/tmp -batch_type tokens \
 -accum_count 4 -dec_layers 2 -rnn_size 512 -word_vec_size 512 \
 -encoder_type transformer_lm -decoder_type transformer_lm -share_embedding \
 -train_steps 10000 -max_generator_batches 4 -dropout 0.1 -normalization tokens \
 -share_vocab -transformer_ff 2048 -max_grad_norm 0 -optim adam -decay_method noam \
 -learning_rate 2 -label_smoothing 0.1 -model_task lm -world_size 1 -gpu_ranks 0 \
 -attention_dropout 0.1 -heads 4 -position_encoding -param_init 0 -warmup_steps 100 \
 -param_init_glorot -adam_beta2 0.998 -src_vocab data/data_lm/data.vocab.src
#
mv /tmp/tmp*10000.pt onmt/tests/test_model_lm.pt
rm /tmp/tmp*.pt
fi
#
if false; then
