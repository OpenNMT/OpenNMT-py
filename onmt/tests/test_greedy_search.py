    ...
    def test_doesnt_predict_eos_if_shorter_than_min_len(self):
        ...
        for batch_sz in [1, 3]:
            ...
            samp = GreedySearch(
                ...
                False, set(), False, 30, 1., 1, 0, 1, False, False)
        ...
        for batch_sz in [1, 13]:
            for temp in [1., 3.]:
                ...
                samp = GreedySearch(
                    ...
                    False, set(), False, 30, temp, 1, 0, 1, False, False)
                samp.initialize(torch.zeros((1, 1)), lengths)
                # initial step
                i = 0
                word_probs = torch.full(
    ...
    def test_returns_correct_scores_non_deterministic(self):
        for batch_sz in [1, 13]:
            for temp in [1., 3.]:
                ...
                samp = GreedySearch(
                    0, 1, 2, 3, batch_sz, GlobalScorerStub(), 0,
                    False, set(), False, 30, temp, 2, 0, 1, False, False)
                samp.initialize(torch.zeros((1, 1)), lengths)
                # initial step
                i = 0
                for _ in range(100):
    ...
    def test_returns_correct_scores_non_deterministic_topp(self):
        for batch_sz in [1, 13]:
            for temp in [1., 0.3]:
                ...
                samp = GreedySearch(
                    0, 1, 2, 3, batch_sz, GlobalScorerStub(), 0,
                    False, set(), False, -1, temp, 50, 0.5, 1, False, False)
                samp.initialize(torch.zeros((1, 1)), lengths)
                # initial step
                i = 0
                for _ in range(100):
