...
class AMRDataset(DatasetBase):
    ...
    @staticmethod
    def make_amr_examples_nfeats_tpl(text_iter, text_path, truncate, 
                                     side, reentrancies):
        """
        Args:
        ...
        if text_iter is None:
            if text_path is not None:
                text_iter = AMRDataset.make_amr_iterator_from_file(text_path)
            else:
                return (None, 0)
        ...
        examples_nfeats_iter = \
            AMRDataset.make_examples(text_iter, truncate, side, reentrancies)
    ...
    def make_examples(text_iter, truncate, side, reentrancies):
        ...
        for i, line in enumerate(text_iter):
            ...
            if side == "tgt":
                words, feats, n_feats = \
                    AMRDataset.extract_text_features(line)
                graph = None
            else:
                feats = None
                n_feats = 0
    ...
    @staticmethod
    def make_amr_iterator_from_file(path):
    ...
    def get_num_features(corpus_file, side):
        ...
        with codecs.open(corpus_file, "r", "utf-8") as cf:
            ...
            if side == "tgt":
                _, _, num_feats = AMRDataset.extract_text_features(f_line)
            else:
                num_feats = 0
