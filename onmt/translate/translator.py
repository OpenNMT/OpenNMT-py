...
class Translator(object):
    ...
    def _fast_translate_batch_without_beam(
        self,
        batch,
        data,
        max_length,
        ...
        mb_device = memory_bank[0].device if isinstance(memory_bank, tuple)\
            else memory_bank.device
        ...
        alive_seq = torch.full(
            ...
            device=mb_device)
        alive_attn = None
        prev_log_probs = torch.zeros([batch_size], dtype=torch.float,
                                     device=mb_device)
        ...
        for step in range(max_length):
            ...
            if is_finished.any():
                ...
                for i in range(is_finished.size(0)):
                    b = batch_offset[i]
                    if is_finished[i]:
                            results["scores"][b].append(
                                top_score[i])
                            results["predictions"][b].append(
                                predictions[i, 1:])
    ...
    def _fast_translate_batch(
        ...
        if self.beam_size == 1:
            return self._fast_translate_batch_without_beam(
                batch, data, max_length, min_length, return_attention)
