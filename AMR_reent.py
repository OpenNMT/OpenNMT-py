...
class AMRData:
    def __init__(self, words, annotation, graph_reent, graph_no_reent):
        self.idx = 0
...
def parse(words, reentrancies):
    ...
    if (-1, 0) not in edges:
        edges = [(-1, 0)] + edges
    return annot_novar, edges_novar, edges

def extract_amr_features(line):
    global i
    if not line:
        return [], []
    words = tuple(line)
    annotation, graph_reent, _ = parse(line, True)
    _, graph_no_reent, _ = parse(line, False)
    return words, AMRData(words, annotation, graph_reent, graph_no_reent)
