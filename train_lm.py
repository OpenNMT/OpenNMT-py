...
def trainModel(model, trainData, validData, dataset, optim, opt):
    ...
    criterion = MLECriterion(dataset['dicts']['src'].size(),
                             len(opt.gpus) > 0)
    ...
    def trainEpoch(epoch):
        ...
        for i in range(len(trainData)):
            ...
            loss, gradOutput, num_correct = memoryEfficientLoss(
                    outputs, targets, model.generator,
                    criterion, opt.max_generator_batches)
            ...
            report_src_words += batch[1].data.sum()
            total_loss += loss
            total_num_correct += num_correct
...
def main():
    ...
    parser.add_argument('-dropout', type=float, default=0.3,
                        help='Dropout probability; applied between LSTM '
                             'stacks.')
    ...
    trainData = onmt.Dataset(dataset['train'],
                             None, opt.batch_size, opt.gpus,
                             data_type=dataset.get("type", "text"))
    validData = onmt.Dataset(dataset['valid'],
                             None, opt.batch_size, opt.gpus,
                             volatile=True,
                             data_type=dataset.get("type", "text"))
    ...
    generator = nn.Sequential(
        nn.Linear(opt.rnn_size, dicts['src'].size()),
        nn.LogSoftmax())
