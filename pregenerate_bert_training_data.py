...
from onmt.utils.bert_tokenization import BertTokenizer, PRETRAINED_VOCAB_ARCHIVE_MAP
from onmt.utils.file_utils import cached_path
import numpy as np
import json
from onmt.inputters.inputter import get_bert_fields, _build_bert_fields_vocab
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                tokens, masked_lm_positions, masked_lm_labels, lm_labels_ids = create_masked_lm_predictions(
...
def main():
    ...
    parser.add_argument('--train_corpus', type=Path, default="/home/lzeng/Documents/OpenNMT-py/onmt/inputters/small_wiki_sentence_corpus.txt")  # required=True)
    parser.add_argument("--corpus_type", type=str, default="train")  # required=True)
    parser.add_argument("--output_dir", type=Path, default="/home/lzeng/Documents/OpenNMT-py/onmt/inputters/test_opennmt/")  # required=True)
    parser.add_argument("--output_name", type=str, default="dataset")
    parser.add_argument("--bert_model", type=str, default="bert-base-uncased")#,  # required=True,
    ...
    parser.add_argument("--max_seq_len", type=int, default=256)  # 128
    ...
    parser.add_argument("--tokens_min_frequency", type=int, default=0)  # not tested 
    parser.add_argument("--vocab_size_multiple", type=int, default=1)  # not tested 

    args = parser.parse_args()
    fields = get_bert_fields()  #
    tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    # save Vocab file
    vocab_file_url = PRETRAINED_VOCAB_ARCHIVE_MAP[args.bert_model]
    vocab_dir = Path.joinpath(args.output_dir, f"{args.bert_model}-vocab.txt")
    vocab_file = cached_path(vocab_file_url, cache_dir=vocab_dir)
    print("Donwload ")
    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        ...
        for epoch in trange(args.epochs_to_generate, desc="Epoch"):
            ...
            epoch_filename = args.output_dir / f"{args.output_name}.{args.corpus_type}.{epoch}.pt"
            json_name = args.output_dir / f"{args.output_name}.{args.corpus_type}.{epoch}.json"                 
            num_instances = 0
            with json_name.open('w') as epoch_file:
                docs_instances = []
                for doc_idx in trange(len(docs), desc="Document"):
                    doc_instances = create_instances_from_document(
                        docs, doc_idx, max_seq_length=args.max_seq_len, short_seq_prob=args.short_seq_prob,
                        masked_lm_prob=args.masked_lm_prob, max_predictions_per_seq=args.max_predictions_per_seq,
                        tokenizer=tokenizer)  # return a list of dict [{}]
                    docs_instances.extend(doc_instances)
                    doc_instances_json = [json.dumps(instance) for instance in doc_instances]
                    for instance in doc_instances_json:
    ...
    if args.corpus_type == "train":
        print("Building vocab from text file...")
